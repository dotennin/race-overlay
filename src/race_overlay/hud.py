...
def _draw_position_marker_arrow(
    ...
    """Draw a navigation arrow marker showing current position and bearing."""
    dx, dy = vector
    ...
    # Arrow dimensions - single-pointed design like navigation apps
    arrow_length = _scale_draw(scale, 24)
    arrow_width = _scale_draw(scale, 8)
    back_length = _scale_draw(scale, 14)
    
    # Arrow tip point (forward)
    tip_x = center_x + (unit_x * arrow_length)
    tip_y = center_y + (unit_y * arrow_length)
    
    # Back point (opposite of tip, forming base of arrow)
    back_x = center_x - (unit_x * back_length)
    back_y = center_y - (unit_y * back_length)
    
    # Perpendicular vectors for width
    side_x = -unit_y
    side_y = unit_x
    
    # Arrow body side points
    left_x = back_x + (side_x * arrow_width)
    left_y = back_y + (side_y * arrow_width)
    
    right_x = back_x - (side_x * arrow_width)
    right_y = back_y - (side_y * arrow_width)
    
    # Simple navigation arrow: single pointed tip with rectangular back
    arrow_points = (
        (tip_x, tip_y),                           # Forward point
        (left_x, left_y),                         # Left back
        (right_x, right_y),                       # Right back
    )
